...
#' Download Data from the World Bank API (supports flows by counterpart area)
#'
...
#' * `"aggregates_only"`
#' * `"all"`
#' @param counterpart Character vector of a counterpart area id or name. Permissible values can be
#' found in the counterparts tibble in [wb_cachelist] or by running [wb_counterparts()] directly.
...
wb_data_flow <- function(indicator, country = "countries_only", counterpart, sources, start_date, end_date,
  ...
  # format counterpart? -----------------------------------------------------
  if(missing(counterpart)){counterpart_path <- NULL}else{
    counterpart_param <- format_wb_counterpart(counterpart, cache = cache)
    counterpart_path <- paste0(wb_api_parameters$counterpart, "/", counterpart_param)}

  # format source ------------------
  if(missing(sources)){
    print("No sources provided. Sources guessed as follows. To override, input source parameters.")
    sources <- guess_wb_source(indicator, cache)
  }else{
    sources <- format_wb_source(sources, indicator, cache)
  }
  ...
  query_list <- list(
    ...
    footnote  = ifelse(return_wide, "y", "n"),
    cntrycode = "y",
    per_page  = 500,#wb_api_parameters$per_page,
    format    = wb_api_parameters$format
  )

  # be able to return this for debugging
  ind_url <- build_wb_series_url(
      base_url  = base_url,  series  = indicator, sources = sources,
...
#   } # end return_wide
#   else {
#     print("where you would reorder columns")
